...
HRESULT DXVertexParticle::createInputLayout(ID3D11Device *device, DXBlob *shaderBlob, ID3D11InputLayout **layout) {

	return device->CreateInputLayout(particleVertexDesc, ARRAYSIZE(particleVertexDesc), shaderBlob->getBufferPointer(), shaderBlob->getBufferSize(), layout);
}
