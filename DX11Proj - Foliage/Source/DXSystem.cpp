...
HRESULT DXSystem::setupWindowDependentResources(HWND hwnd) {
	...
	depthStencilDesc.Format = DXGI_FORMAT_R24G8_TYPELESS;// DXGI_FORMAT_D24_UNORM_S8_UINT;
	...
	if (SUCCEEDED(hr))
		hr = device->CreateDepthStencilView(depthStencilBuffer, &descDSV, &depthStencilReadOnlyView);
...
ID3D11ShaderResourceView* DXSystem::getDepthStencilSRV(){
	return depthStencilSRV;
}
ID3D11DepthStencilView* DXSystem::getDepthStencilReadOnlyView(){
